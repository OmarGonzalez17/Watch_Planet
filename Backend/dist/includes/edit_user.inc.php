 <?php
    if (isset($_POST['edit-submit'])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z]*$/", $name) && !preg_match("/^[a-zA-Z]*$/", $last_name)) {
            header("Location: ../users.php?error=invalidinputs");
            exit();
        }
        else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            header("Location: ../users.php?error=invalidemail&name=".$name."&last_name=".$last_name);
            exit();
        }
        else if (!preg_match("/^[a-zA-Z]*$/", $name)) {
            header("Location: ../users.php?error=invalidname&last_name=".$last_name."&email=".$email);
            exit();
        }
        else if (!preg_match("/^[a-zA-Z]*$/", $last_name)) {
            header("Location: ../users.php?error=invalidlast_name&name=".$name."&email=".$email);
            exit();
        }
        else if ($pass !== $pass_confirm) {
            header("Location: ../users.php?error=passwordcheck&name=".$name."&last_name=".$last_name."&email=".$email);
            exit();
        }
        else {
